...
export default function NewfeedsScreen() {
  ...
  useFocusEffect(
    React.useCallback(() => {
      ...
      lastScrollY.value = 0;
    }, [headerOpacity, headerTranslateY, lastScrollY]),
  );
  ...
  const onScroll = useAnimatedScrollHandler({
    onScroll: (event) => {
      ...
      if (nextOffset <= TOP_THRESHOLD) {
        headerTranslateY.value = withTiming(0, { duration: HEADER_SHOW_DURATION });
        headerOpacity.value = withTiming(1, { duration: HEADER_SHOW_DURATION });
      ...
      if (delta > DELTA_THRESHOLD) {
        headerTranslateY.value = withTiming(-headerHeight, { duration: HEADER_HIDE_DURATION });
        headerOpacity.value = withTiming(0, { duration: HEADER_HIDE_DURATION });
